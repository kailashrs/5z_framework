  ...
  extends AutoCompleteTextView
{
  ...
  public boolean enoughToFilter()
  {
    ...
    if ((i >= 0) && (mTokenizer != null)) {
      return i - mTokenizer.findTokenStart(localEditable, i) >= getThreshold();
    }
    return false;
  }
  ...
  protected void performFiltering(CharSequence paramCharSequence, int paramInt)
  {
    ...
    {
      int i = getSelectionEnd();
      performFiltering(paramCharSequence, mTokenizer.findTokenStart(paramCharSequence, i), i, paramInt);
    }
    else
    {
      dismissDropDown();
  ...
  public void performValidation()
  {
    ...
    if ((localValidator != null) && (mTokenizer != null))
    {
      Editable localEditable = getText();
      int j;
      for (int i = getText().length(); i > 0; i = j)
      {
        ...
        CharSequence localCharSequence = localEditable.subSequence(j, mTokenizer.findTokenEnd(localEditable, j));
        if (TextUtils.isEmpty(localCharSequence)) {
          localEditable.replace(j, i, "");
        } else if (!localValidator.isValid(localCharSequence)) {
          localEditable.replace(j, i, mTokenizer.terminateToken(localValidator.fixText(localCharSequence)));
        }
      }
      return;
    }
  ...
  protected void replaceText(CharSequence paramCharSequence)
  {
    ...
    localEditable.replace(j, i, mTokenizer.terminateToken(paramCharSequence));
  }
