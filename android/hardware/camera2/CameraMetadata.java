...
public abstract class CameraMetadata<TKey>
{
  ...
  private static <TKey> boolean shouldKeyBeAdded(TKey paramTKey, Field paramField, int[] paramArrayOfInt)
  {
    if (paramTKey != null)
    {
      if ((paramTKey instanceof CameraCharacteristics.Key)) {
        paramTKey = ((CameraCharacteristics.Key)paramTKey).getNativeKey();
      }
      for (;;)
      {
        break;
        if ((paramTKey instanceof CaptureResult.Key))
        {
          paramTKey = ((CaptureResult.Key)paramTKey).getNativeKey();
        }
        else
        {
          if (!(paramTKey instanceof CaptureRequest.Key)) {
            break label108;
          }
          paramTKey = ((CaptureRequest.Key)paramTKey).getNativeKey();
        }
      }
      if (paramField.getAnnotation(PublicKey.class) == null) {
        return false;
      }
      boolean bool = true;
      ...
      if (paramField.getAnnotation(SyntheticKey.class) != null) {
        return true;
      }
      if (Arrays.binarySearch(paramArrayOfInt, paramTKey.getTag()) < 0) {
        bool = false;
      }
      return bool;
      label108:
  ...
  <TKey> ArrayList<TKey> getKeys(Class<?> paramClass, Class<TKey> paramClass1, CameraMetadata<TKey> paramCameraMetadata, int[] paramArrayOfInt)
  {
    Object localObject = paramClass;
    if (paramClass.equals(TotalCaptureResult.class)) {
      localObject = CaptureResult.class;
    }
    if (paramArrayOfInt != null) {
    ...
    for (Field localField : ((Class)localObject).getDeclaredFields()) {
      if ((localField.getType().isAssignableFrom(paramClass1)) && ((localField.getModifiers() & 0x1) != 0)) {
        try
        {
          paramClass = localField.get(paramCameraMetadata);
          if (((paramCameraMetadata == null) || (paramCameraMetadata.getProtected(paramClass) != null)) && (shouldKeyBeAdded(paramClass, localField, paramArrayOfInt))) {
            localArrayList.add(paramClass);
          }
        }
        catch (IllegalArgumentException paramClass)
        {
          throw new AssertionError("Can't get IllegalArgumentException", paramClass);
        }
        catch (IllegalAccessException paramClass)
        {
          throw new AssertionError("Can't get IllegalAccessException", paramClass);
        }
      }
    }
    ...
    paramClass = mNativeInstance.getAllVendorKeys(paramClass1);
    if (paramClass != null)
    {
      paramClass1 = paramClass.iterator();
      while (paramClass1.hasNext())
      {
        paramCameraMetadata = paramClass1.next();
        long l;
        if ((paramCameraMetadata instanceof CaptureRequest.Key))
        {
          paramClass = ((CaptureRequest.Key)paramCameraMetadata).getName();
          l = ((CaptureRequest.Key)paramCameraMetadata).getVendorId();
        }
        for (;;)
        {
          break label282;
          if ((paramCameraMetadata instanceof CaptureResult.Key))
          {
            paramClass = ((CaptureResult.Key)paramCameraMetadata).getName();
            l = ((CaptureResult.Key)paramCameraMetadata).getVendorId();
          }
          else
          {
            if (!(paramCameraMetadata instanceof CameraCharacteristics.Key)) {
              break;
            }
            paramClass = ((CameraCharacteristics.Key)paramCameraMetadata).getName();
            l = ((CameraCharacteristics.Key)paramCameraMetadata).getVendorId();
          }
        }
        label282:
        if ((paramArrayOfInt == null) || (Arrays.binarySearch(paramArrayOfInt, CameraMetadataNative.getTag(paramClass, l)) >= 0)) {
          localArrayList.add(paramCameraMetadata);
        }
      }
    }
    return localArrayList;
  ...
  public List<TKey> getKeys()
  {
    ...
    return Collections.unmodifiableList(getKeys(localClass, getKeyClass(), this, null));
  }
  
  protected abstract <T> T getProtected(TKey paramTKey);
  
  protected void setNativeInstance(CameraMetadataNative paramCameraMetadataNative)
  {
    mNativeInstance = paramCameraMetadataNative;
  }
}
